...
import {QuestionAnswer, ResponseRecord, ResponseUserRecord} from "../../../features/types";
    ...
    try {
      ...
      const userResponses = formResponses.map((resp) => {
        return {
          ...
          userResponse: userResponsesCollection.data.find(r => r.data.responseId === resp.data.id && r.data.userId === auth.accountAddress),
      ...
      const decryptedResponses = userResponses.map(async (resp) => {
        ...
        const symmetricKey = await secp256k1.asymmetricDecryptFromEncoding(decodedPrivateKey, resp?.userResponse?.data.encryptedEncryptionKey as string, 'base64');
        return await aescbc.symmetricDecryptFromEncoding(decodeFromString(symmetricKey, 'hex'), resp.encryptedData, 'base64')
      });

      const parsedResponses = await Promise.all(decryptedResponses);
      setDecryptedAnswers(parsedResponses.map(r => JSON.parse(r) as QuestionAnswer[]));
      setLoading(false)
    } catch (e) {
      console.log(e)
    }
        ...
        return (
          <Box bg='red.200' mb={11} key={index}>
            {answers.map((answer, index) => {
              return <Text color='white' key={index}>{answer.data}</Text>
            })}
          </Box>
        )
      })}
